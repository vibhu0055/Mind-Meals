...
export const login = async (req, res) => {
  try {
    ...
    // 🔹 PRINCIPAL / ADMIN (mapped to school)
    if (role === 'principal' || role === 'admin') {
      ...
      const token = generateToken({ school_id: school.id, role });
    ...
    if (role === 'teacher') {
      ...
      return res.status(200).json({
        ...
        user: {
          ...
          role: 'teacher',
          school_id: teacher.school_id,
          can_manage_meals: teacher.can_manage_meals
        }
      });
    }

    return res.status(400).json({
      message: 'role must be principal, admin or teacher'
    });
